 ...
 */
export function exportPaymentCsv(
  ...
  const rows = payrolls.map((p) => {
    const workerName = [
      p.worker?.name,
      p.worker?.paternalLastName,
      p.worker?.maternalLastName,
    ]
      .filter(Boolean)
      .join(" ");
 ...
 */
export function createLiquidacionHtmlElement(
  p: PayrollExportData,
): HTMLElement {
  ...
  const taxableItems =
    p.details?.filter((d) => d.conceptType === "TAXABLE_INCOME") || [];
  const nonTaxableItems =
    p.details?.filter((d) => d.conceptType === "NON_TAXABLE_INCOME") || [];
  const legalDeductions =
    p.details?.filter((d) => d.conceptType === "LEGAL_DEDUCTION") || [];
  const otherDeductions =
    p.details?.filter((d) => d.conceptType === "OTHER_DEDUCTION") || [];
        ...
        <div style="padding: 12px; line-height: 1.6;">
          ...
          ${
            nonTaxableItems.length === 0
              ? `<p style="color: #a3a3a3; font-style: italic; font-size: 11px; margin: 0;">Sin haberes no imponibles</p>`
              : nonTaxableItems
                  .map(
                    (item) => `
            <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
        ...
        <div style="padding: 12px; line-height: 1.6;">
          ...
          ${
            otherDeductions.length === 0
              ? `<p style="color: #a3a3a3; font-style: italic; font-size: 11px; margin: 0;">Sin otros descuentos</p>`
              : otherDeductions
                  .map(
                    (item) => `
            <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
...
export async function generateUnifiedPdf(
  ...
  // Mantener el contenedor fuera de la pantalla pero renderizable
...
export async function generateZipOfPdfs(
  ...
  for (let i = 0; i < payrolls.length; i++) {
    ...
    try {
      ...
      const safeRut = (p.worker?.rut || `Worker_${i + 1}`).replace(
        /[^a-zA-Z0-9_-]/g,
        "",
      );
